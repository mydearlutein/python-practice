...
def extract_data(source_dir, file_groups, target_format):
    ...
    for format_type, files in file_groups.items():
        ...
        for file_name in files:
            data={'file_name': "", 'source_format': format_type, 'target_format': target_format, 'page_separated': _separated, 'pages':[], 'context': ""}
            ...
            if target_format=="html" and data['context']:
                with open(os.path.join(new_dir, "({}){}.html".format(format_type, "".join(file_name.split(".")[:-1]))), "w", encoding="utf-8") as f:
                    f.write(data['context'])


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='File parser test')
    parser.add_argument('--source_format', help='Source file format (pdf,docx,hwp)',required=False, default="")
    parser.add_argument('--source_dir', help='Target file directory')
    parser.add_argument('--target_format', help='Target file format (text,html)')
    parser.add_argument('--log_file',help='Log file full path',required=False, default="")
    args = parser.parse_args()

    source_dir = args.source_dir
    source_format = args.source_format
    target_format = args.target_format
    log_file = source_dir + args.log_file if args.log_file else LOG_FILE
